...
def call_openai_chat(
    ...
    # Gemini-compatible endpoint; final reaction yield-rate answer is expected in message.content.
    payload = {
        "model": model,
    ...
    content_text = stringify_content(message.get("content"))
    if not content_text:
        content_text = stringify_content(choice.get("text"))
    if not content_text:
        content_text = stringify_content(data.get("output_text"))

    reasoning_text = stringify_content(
        message.get("reasoning_content") or message.get("reasoning")
    )

    # If an OpenAI-compatible Gemini proxy returns an unfamiliar schema or an empty
    # content field, keep the raw JSON in reasoning_output for debugging. Do not parse
    # metadata as an answer, because keys such as finish_reason can cause false matches.
    if not content_text:
        raw_response = json.dumps(data, ensure_ascii=False)
        reasoning_text = "\n".join(part for part in (reasoning_text, raw_response) if part)

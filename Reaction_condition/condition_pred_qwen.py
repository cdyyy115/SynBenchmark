...
LIST_PREFIX_CHARS = "-*•"
LABEL_RE = re.compile(
    r"^\s*(?:final\s+answer|answer|predicted\s+conditions(?:\s+smiles)?|conditions(?:\s+smiles)?|predicted\s+reagents(?:\s+smiles)?|reagents(?:\s+smiles)?|solvents(?:\s+smiles)?|catalysts(?:\s+smiles)?|smiles)\s*[:：]\s*",
    flags=re.IGNORECASE,
)
...
def _json_candidates(text: str):
    ...
    if isinstance(obj, str):
        ...
        for key in ("smiles", "condition_smiles", "conditions_smiles", "conditions", "reagents_smiles", "reagents", "solvents", "catalysts", "answer", "candidates"):
            value = obj.get(key)
...
def extract_candidates(text: object, max_candidates: int):
    """Extract only RDKit-valid condition SMILES from model output."""
    if text is None:
        return []
...
def build_prompt(prompt: str, candidates_per_call: int) -> str:
    ...
    if n == 1:
        output_rule = (
            "Return ONLY the predicted reagents, solvents, and catalysts SMILES on a single line. "
            "Do not include explanations, labels, Markdown, numbering, or any other text."
        )
    else:
        output_rule = (
            f"Return ONLY {n} predicted reagents, solvents, and catalysts SMILES strings, one candidate per line. "
            "Do not include explanations, labels, Markdown, numbering, or any other text."
        )
...
def call_openai_chat(
    ...
    # Qwen-compatible endpoint; final reaction condition answer is expected in message.content.
    payload = {
    "model": model,
...
def is_successful_existing_row(row):
    ...
    if not row:
        return False
    pred = str(row.get("pred_conditions", "")).strip()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run reaction condition generation prompts with an OpenAI-compatible API, with RDKit SMILES validation."
    )
    parser.add_argument("--input-jsonl", type=Path, required=True)
    parser.add_argument("--output-csv", type=Path, required=True)
    parser.add_argument("--model", default=os.getenv("OPENAI_MODEL", "qwen3"))
    parser.add_argument(
        "--base-url",
        default=os.getenv("OPENAI_BASE_URL", "https://www.autodl.art/api/v1"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="How many condition SMILES strings the prompt requests from each API call.",
    )
        ...
        for run_id in range(1, args.num_runs + 1):
            ...
            try:
                ...
                if not candidates:
                    ...
                    raise ValueError(
                        "Model returned no RDKit-valid condition SMILES candidates. "
                        f"Last content preview: {preview!r}"
                    )

                result["pred_conditions"] = candidates[0]
                result["pred_conditions_list"] = " ||| ".join(candidates)
                ...
                result["raw_output"] = "\n---CALL---\n".join(raw_outputs)
                result["reasoning_output"] = "\n---CALL---\n".join(reasoning_outputs)
                result["pred_conditions"] = candidates[0] if candidates else ""
                result["pred_conditions_list"] = " ||| ".join(candidates)
                result["num_candidates"] = len(candidates)
                result["num_calls"] = len(raw_outputs)
                result["candidates_per_call"] = args.candidates_per_call

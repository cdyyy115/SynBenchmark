...
LIST_PREFIX_CHARS = "-*•"
LABEL_RE = re.compile(
    r"^\s*(?:final\s+answer|answer|predicted\s+reactants(?:\s+smiles)?|reactants(?:\s+smiles)?|predicted\s+product(?:\s+smiles)?|product(?:\s+smiles)?|smiles)\s*[:：]\s*",
    flags=re.IGNORECASE,
)
...
def _json_candidates(text: str):
    ...
    if isinstance(obj, str):
        ...
        for key in ("smiles", "reactants_smiles", "reactants", "precursors", "answer", "candidates", "product_smiles", "product", "products"):
            value = obj.get(key)
...
def extract_candidates(text: object, max_candidates: int):
    """Extract only RDKit-valid reactants/reagents SMILES from model output."""
    if text is None:
        return []
...
def build_prompt(prompt: str, candidates_per_call: int) -> str:
    ...
    if n == 1:
        ...
        output_rule = (
            f"Return ONLY {n} predicted reactants SMILES, one SMILES per line. "
            "Do not include explanations, labels, Markdown, numbering, or any other text."
        )
...
def call_openai_chat(
    ...
    # Qwen-compatible endpoint; final retrosynthesis answer is expected in message.content.
    payload = {
    "model": model,
...
def is_successful_existing_row(row):
    ...
    if not row:
        return False
    pred = str(row.get("pred_reactants", "")).strip()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run retrosynthesis prompts with an OpenAI-compatible API, with RDKit SMILES validation."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="How many reactants SMILES strings the prompt requests from each API call.",
    )
        ...
        for run_id in range(1, args.num_runs + 1):
            ...
            try:
                ...
                if not candidates:
                    ...
                    raise ValueError(
                        "Model returned no RDKit-valid reactants SMILES candidates. "
                        f"Last content preview: {preview!r}"
                    )

                result["pred_reactants"] = candidates[0]
                result["pred_reactants_list"] = " ||| ".join(candidates)
                ...
                result["raw_output"] = "\n---CALL---\n".join(raw_outputs)
                result["reasoning_output"] = "\n---CALL---\n".join(reasoning_outputs)
                result["pred_reactants"] = candidates[0] if candidates else ""
                result["pred_reactants_list"] = " ||| ".join(candidates)
                result["num_candidates"] = len(candidates)
                result["num_calls"] = len(raw_outputs)
                result["candidates_per_call"] = args.candidates_per_call
